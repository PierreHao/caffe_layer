...
void PPReLULayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > filler1;
    if (prelu_param.has_filler()) {
      filler.reset(GetFiller<Dtype>(prelu_param.filler()));
      filler1.reset(GetFiller<Dtype>(prelu_param.filler()));
    } else {
      ...
      filler_param1.set_type("constant");
      filler_param1.set_value(1);
      filler1.reset(GetFiller<Dtype>(filler_param1));
    }
    filler->Fill(this->blobs_[0].get());
    filler1->Fill(this->blobs_[1].get());
  }
  if (channel_shared_) {
  ...
  backward_buff_1.Reshape(vector<int>(1, bottom[0]->count(1)));
...
void PPReLULayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* slope_data1 = this->blobs_[1]->cpu_data();

  // For in-place computation
  if (bottom[0] == top[0]) {
    caffe_copy(count, bottom_data, bottom_memory_.mutable_cpu_data());
  }
  //std::cout << "forward "<< slope_data[0] << "," << slope_data1[0]<< "," << bottom_data[0] << std::endl;
  // if channel_shared, channel index in the following computation becomes
  // always zero.
  const int div_factor = channel_shared_ ? channels : 1;
  for (int i = 0; i < count; ++i) {
    int c = (i / dim) % channels / div_factor;
    if (bottom_data[i] < Dtype(4)){
      top_data[i] = bottom_data[i] * (bottom_data[i] > Dtype(0))
        + slope_data[c] * bottom_data[i] * (bottom_data[i] <= Dtype(0));
    }
    else{
      top_data[i] = slope_data1[c] * bottom_data[i];
    }
    /*
    if (bottom_data[i] > Dtype(4))
        top_data[i] = slope_data1[c] * bottom_data[i];
      ...
      if (bottom_data[i] < Dtype(-4))
        top_data[i] = slope_data[c] * bottom_data[i];
      else
...
void PPReLULayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    Dtype* slope_diff1 = this->blobs_[1]->mutable_cpu_diff();
    for (int i = 0; i < count; ++i) {
      int c = (i / dim) % channels / div_factor;
      /*
      if (bottom_data[i] > Dtype(4))
        slope_diff1[c] += top_diff[i] * bottom_data[i];
      else{
    ...
    //std::cout<< "Backward "<<slope_diff[0] << "," << slope_diff1[0] << std::endl;
  }
  // Propagate to bottom
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      if (bottom_data[i] < Dtype(4)){
        ...
        bottom_diff[i] = top_diff[i] * slope_data1[c];
      }
      /*
      if (bottom_data[i] > Dtype(4))
        bottom_diff[i] = slope_data1[c];
      else{
        if (bottom_data[i] < Dtype(0))
          bottom_diff[i] = slope_data[c];
        else
          bottom_diff[i] = top_diff[i];
      }
      */
  }
  //std::cout << "bottom diff "<<bottom_diff[0] << std::endl;
}
}
